const CACHE='laxman-apps-v5';
const CORE=['/apps/','/apps/launcher-enhancements.js','/apps/launcher-next.js','/apps/launcher-cards.js','/manifest.webmanifest','/assets/app-icon.svg','/assets/css/style.css','/assets/css/light.css','/assets/css/gadgetbyte-home.css'];
...
async function inject(response){
 ...
 try{
  ...
  const scripts=['launcher-enhancements.js?v=5','launcher-next.js?v=5','launcher-cards.js?v=5'];
...
self.addEventListener('fetch',e=>{
 ...
 e.respondWith((async()=>{
  ...
  try{
   ...
   if(u.pathname==='/apps/'){
    const enhanced=await inject(fresh);caches.open(CACHE).then(c=>c.put(e.request,enhanced.clone()));return enhanced;
   }
