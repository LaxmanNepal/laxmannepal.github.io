...
def modernize(path: Path, meta: dict):
    ...
    article = f'''<body>{HEADER}<main class="modern-article-main"><div class="modern-breadcrumbs"><a href="/">Laxman Nepal</a> / Blog / Article</div><div class="modern-article-kicker"><span class="modern-pill">ARTICLE</span>{date_label}</div><h1>{title_esc}</h1><p class="modern-dek">{desc_esc}</p><article class="modern-reading-card">{inner}</article><section class="modern-related"><strong>Continue exploring</strong><br><a href="/search/">Search more Laxman Nepal articles →</a><a href="/tools/">Explore free tools →</a><a href="/ai/">Explore AI resources →</a></section></main>{FOOTER}</body>'''
...
def main():
    ...
    for meta in data.get("posts",[]):
        ...
        if not rel: continue
        p=ROOT/rel
        if p.is_file() and modernize(p,meta): changed+=1
