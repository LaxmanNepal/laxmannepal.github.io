...
# The global search must remain keyboard- and screen-reader-accessible.
intel = (ROOT / 'assets' / 'site-intelligence.js').read_text(encoding='utf-8')
for needle in ('aria-label=\\\'Search Laxman Nepal\\\'', 'aria-modal', 'role=\\\'dialog\\\'', 'aria-label=\\\'Close search\\\'', 'type=\\\'search\\\''):
    if needle not in intel:
        errors.append(f'global search accessibility marker missing: {needle}')

# App directory must have exactly one enhancement script.
