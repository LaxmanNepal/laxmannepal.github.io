...
def update_snapshot(html, latest):
    ...
    html = re.sub(r'<meta name="dateModified"[^>]*>', '', html)
    ...
    html = re.sub(r'(<span>Fine Gold \(9999\) per tola</span><b>)Rs [0-9,]+(</b>)', rf'\g<1>Rs {latest["fine_gold_tola"]:,}\g<2>', html, count=1)
    html = re.sub(r'(<span>Tejabi Gold per tola</span><b>)Rs [0-9,]+(</b>)', rf'\g<1>Rs {latest["tejabi_gold_tola"]:,}\g<2>', html, count=1)
    html = re.sub(r'(<span>Silver per tola</span><b>)Rs [0-9,]+(</b>)', rf'\g<1>Rs {latest["silver_tola"]:,}\g<2>', html, count=1)
    return html
...
def main():
    ...
    page = update_snapshot(page, record)
    page = page.replace(
        'FENEGOSIDA published rate feed',
        SOURCE_NAME
    )
    page = page.replace(
        'https://www.ashesh.com.np/gold/widget.php?api=872076p237&header_color=0077e5',
        SOURCE_URL
    )
