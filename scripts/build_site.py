...
for a in articles:
 content=md(a['source']); words=len(re.findall(r"\b[\w’'-]+\b",re.sub('<[^>]+>',' ',content))); minutes=max(1,round(words/220)); tags=''.join(f'<a class="tag" href="/blog/?tag={esc(t)}">{esc(t)}</a>' for t in a['tags'])
 schema_data={'@context':'https://schema.org','@type':'Article','headline':a['title'],'description':a['description'],'datePublished':a['date'],'dateModified':a['updated'],'author':{'@type':'Person','name':a['author']},'mainEntityOfPage':{'@type':'WebPage','@id':a['url']}}
 if a.get('image'): schema_data['image']=image_url(a['image'])
 schema='<script type="application/ld+json">'+json.dumps(schema_data,ensure_ascii=False)+'</script>'
 hero=''
 if a.get('image'):
  hero=f'<figure class="article-hero"><img src="{esc(a["image"])}" alt="{esc(a.get("image_alt",a["title"]))}" width="1200" height="675" loading="eager" decoding="async"><figcaption>{esc(a.get("image_caption","Product image used for reference; specifications and prices should be checked before purchase."))}</figcaption></figure>'
 body=f'<div class="article-wrap"><article><nav class="breadcrumbs"><a href="/">Home</a> / <a href="/blog/">Blog</a> / {esc(a["title"])}</nav><header class="article-header"><p class="eyebrow">{esc(a["category"])} · {minutes} min read</p><h1>{esc(a["title"])}</h1><p class="article-description">{esc(a["description"])}</p><div class="article-meta">By {esc(a["author"])} · Published {a["date"]} · Updated {a["updated"]}</div><div class="tags">{tags}</div></header>{hero}<div class="article-content">{content}</div></article></div>'
 out=OUT/a['slug']/ 'index.html'; out.parent.mkdir(parents=True,exist_ok=True); out.write_text(shell(a['title'],a['description'],a['url'],body,'Article',a.get('image','')).replace('</head>',schema+'</head>'),encoding='utf-8')
cards=''.join(f'<article class="post-card" data-tags="{esc(" ".join(a["tags"]))}" data-category="{esc(a["category"])}">{("<img src=\""+esc(a["image"])+"\" alt=\""+esc(a.get("image_alt",a["title"]))+"\" loading=\"lazy\" decoding=\"async\">") if a.get("image") else ""}<p class="eyebrow">{esc(a["category"])} · {a["date"]}</p><h2><a href="/blog/{a["slug"]}/">{esc(a["title"])}</a></h2><p>{esc(a["description"])}</p><a class="text-link" href="/blog/{a["slug"]}/">Read article →</a></article>' for a in sorted(articles,key=lambda x:x['date'],reverse=True))
b=f'<div class="article-wrap"><section class="blog-hero"><p class="eyebrow">Laxman Nepal Journal</p><h1>Useful ideas.<br><em>Explained simply.</em></h1><p>Practical guides about technology, AI, apps, productivity and digital work.</p><input id="blogFilter" class="search-box" placeholder="Filter by tag or category…" aria-label="Filter articles"></section><section class="post-grid" id="postGrid">{cards or "<p>No articles published yet.</p>"}</section></div><script>const q=new URLSearchParams(location.search).get("tag");const i=document.querySelector("#blogFilter");if(q&&i)i.value=q;function f(){const v=(i?.value||"").toLowerCase().trim();document.querySelectorAll("#postGrid .post-card").forEach(c=>c.hidden=v&&!((c.dataset.tags+" "+c.dataset.category).toLowerCase().includes(v)))}i?.addEventListener("input",f);f();</script>'
(OUT/'index.html').parent.mkdir(parents=True,exist_ok=True); (OUT/'index.html').write_text(shell('Blog — Laxman Nepal','Practical technology, AI, apps and productivity articles by Laxman Nepal.',BASE+'/blog/',b),encoding='utf-8')
