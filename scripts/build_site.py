...
def shell(title,desc,url,body,kind='WebPage',schema_extra=None):
...
def main():
 ...
 for a in articles:
  ...
  d=OUT/a['slug']; d.mkdir(parents=True,exist_ok=True); (d/'index.html').write_text(shell(a['title'],a['description'],url,body,'Article',{'datePublished':a['date'],'dateModified':a['updated'],'breadcrumb':breadcrumbs}),encoding='utf-8')
