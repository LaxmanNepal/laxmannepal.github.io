...
def entries_from_cell(cell):
    text=clean(cell.get_text(' ',strip=True)).replace('NPR','NPR ')
    pattern=r'(?:NPR|Rs\.?)[ \t]*([\d,]+)(?:[ \t]+([\d,]+))?[ \t]*\(([^)]*)\)'
    entries=[]
    for m in re.finditer(pattern,text,re.I):
        values=[int(x.replace(',','')) for x in m.groups()[:2] if x]
        if not values:continue
        d={'price':values[-1],'variant':clean(m.group(3))}
        if len(values)>1:d['original_price']=values[-2]
        entries.append(d)
    # Fallback for rows without a parenthesized variant.
    if not entries:
        for raw in re.findall(r'(?:NPR|Rs\.?)[ \t]*[\d,]+(?:[ \t]+[\d,]+)?',text,re.I):
            nums=[int(x.replace(',','')) for x in re.findall(r'[\d,]+',raw)]
            if nums:
                d={'price':nums[-1],'variant':''}
                if len(nums)>1:d['original_price']=nums[-2]
                entries.append(d)
    return entries
...
def parse_catalog(html):
    ...
    products=[]
    for p in out.values():
        p['variants']=list(dict.fromkeys(p['variants']));p['prices']=sorted(set(p['prices']));p['price']=min(p['prices']);p['max_price']=max(p['prices']);p['original_price']=max(p['original_prices'] or [0]) or None;p['rams']=sorted(set(re.findall(r'\b\d+GB\b',' '.join(p['variants']))),key=lambda x:int(re.search(r'\d+',x).group()));p['storages']=sorted(set(re.findall(r'\b(?:\d+GB|\d+TB)\b',' '.join(p['variants']))),key=lambda x:int(re.search(r'\d+',x).group())*(1024 if 'TB' in x else 1));p['is_5g']=bool(re.search(r'\b5G\b',' '.join([p['name']]+p['variants']),re.I));p.pop('prices',None);p.pop('original_prices',None);products.append(p)
    return products
